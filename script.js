...
var app = new Vue({
    ...
    computed: {
        time2() {
            let matches = [];
            for (let match in this.time1) matches.push([this.time1[match][1], this.time1[match][0]]);
            return matches;
        },
        matches() {
    ...
    methods: {
        ...
        goBack: function () {
            this.go(this.safe_url + '?' + this.$route.params.id + '#/' + this.$route.params.id)
        },
        goHome: function () {
            let url = window.location.href.split('#')[0];
            url = url.split('?')[0] + "?" + "#/";
        ...
        join: function () {
            if (this.getid && this.getid.length > 5) this.go(this.safe_url + '?'+ this.getid + '#/' + this.getid)
            else {
        ...
        del: function () {
            let ok = confirm(this.league[0].toUpperCase() + this.league.slice(1) + " ligi silinsin mi?");
            if (ok == true) {
                axios.delete("https://lig123.firebaseio.com/id/" + this.$route.params.id + "/"+this.name+".json").then(obj => {
                    this.goBack();
                });
            }
        },
        ...
        getScores: function () {
            ...
            for (let i = 0; i < items.length; i++) scores.push(items[i].getElementsByTagName('input')[0].value + "-" + items[i].getElementsByTagName('input')[1].value);
            this.scores = scores;
        },
        suffle: function (a) {
            let j, x, i;
        ...
        sort: function () {
            let s = this, points = {}, samePoints = {}, same_index = [], mac = [], sort = [], inde = 0, ip = 0,
                i = 0, x = -1, rArr = {}, ind = 0;
            ...
            for (let item in same_index) {
                let ilks = [], ritem = same_index[item].split("-");
            ...
            for (let item in mac) {
                ...
                let goal_i = parseInt(document.getElementById('#' + mac[item]).document.getElementsByClassName("in")[0].value),
                    goal_o = parseInt(document.getElementById('#' + mac[item]).document.getElementsByClassName("out")[0].value);

                if (goal_i > -1 && goal_o > -1) {

                    // GET ID
                    let id_i = parseInt(mac[item].split("_")[0]), id_o = parseInt(mac[item].split("_")[1]);
        ...
        engine: function (s) {
            ...
            if (s.$route.params.league) {
                axios.get("https://lig123.firebaseio.com/id/" + s.$route.params.id + "/" + s.$route.params.league + ".json")
                    .then(response => {
                        ...
                        if (s.scores.toString() !== data.scores.toString()) {
                            ...
                            setTimeout(function () { s.load = true }, 200);
                            s.name = data.name;
                            s.values.o = arr;
                            s.values.a = arr;
                            s.values.y = arr;
                    ...
                    .catch(error => {
                        ...
                        let teams = s.teams, matches = [];
                        for (let item in teams) {
                            for (let item2 in teams) {
                                if (item !== item2) {
                                    let nohave = 0;
                                    if (matches) for (let match in matches) if ((matches[match][0] === item && matches[match][1] === item2) || (matches[match][0] === item2 && matches[match][1] === item)) nohave++;
                        ...
                        let time = new Date().getTime(), scores = false;
                        if (s.scores.length) scores = s.scores;

                        axios.put("https://lig123.firebaseio.com/id/" + s.$route.params.id + "/"+time+".json", {
                            name: time,
                            league: s.league,
                            scores: scores,
                            teams: s.teams,
            ...
            if (s.$route.params.id && !s.$route.params.league) {
                axios.get("https://lig123.firebaseio.com/id/" + s.$route.params.id + ".json")
                    .then(response => {
                        let object = response.data, newObject = {}, keys = [];
    ...
    watch: {
        scores(val) {
            ...
            for (let match in matches) {
                try {
                    ...
                    if (ilk && son) {
                        ...
                        if (parseInt(ilk) > parseInt(son)) s.values.p[matches[match][0]] += 3;
                        if (parseInt(ilk) < parseInt(son)) s.values.p[matches[match][1]] += 3;
            ...
            axios.put("https://lig123.firebaseio.com/id/" + this.$route.params.id + "/"+s.name+".json", {
                name: s.name,
                league: s.league,
                scores: s.scores,
                teams: s.teams,
                ...
                clearInterval(s.up);
                s.up = setInterval(function () { s.engine(s) }, 5000)
            });
        }
    },
    mounted() {
